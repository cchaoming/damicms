...
class Backup extends Common
{
    ...
    {
        $list = Db::query("SHOW TABLES FROM " . "`" . config('database.connections.mysql.database') . "`");
        $table = array();
        foreach ($list as $k => $v) {
            $table[$k] = current($v);
        }
    ...
    public function dobackup()
    {
        if ($this->request->isPost()) {
            ...
            foreach ($_POST['ids'] as $table) {
                $rs = M($table, false, false);
                $array = $rs->select()->toArray();
    ...
    {
        $pre = $this->request->param('id');
        $rd = $this->request->param('rd');
        $fileid = is_null($this->request->param('fileid')) ? intval($this->request->param('fileid')) : 1;
