...
class RBAC
{
    ...
    {
        if (empty($model)) $model = config('app.USER_AUTH_MODEL');
        //使用给定的Map进行认证
    ...
    {
        if (null === $authId) $authId = session(config('app.USER_AUTH_KEY'));
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
        if (config('app.USER_AUTH_TYPE') != 2 && !session(config('app.ADMIN_AUTH_KEY'))) {
    ...
    static function checkAccess()
    {
        ...
        if (config('app.USER_AUTH_ON')) {
            $_module = array();
            $_action = array();
            if ("" != config('app.REQUIRE_AUTH_MODULE')) {
                //需要认证的模块
                $_module['yes'] = explode(',', strtoupper(config('app.REQUIRE_AUTH_MODULE')));
            } else {
                //无需认证的模块
                $_module['no'] = explode(',', strtoupper(config('app.NOT_AUTH_MODULE')));
            }
            //检查当前模块是否需要认证
            if ((!empty($_module['no']) && !in_array(strtoupper(MODULE_NAME), $_module['no'])) || (!empty($_module['yes']) && in_array(strtoupper(MODULE_NAME), $_module['yes']))) {
                if ("" != config('app.REQUIRE_AUTH_ACTION')) {
                    //需要认证的操作
                    $_action['yes'] = explode(',', strtoupper(config('app.REQUIRE_AUTH_ACTION')));
                } else {
                    //无需认证的操作
                    $_action['no'] = explode(',', strtoupper(config('app.NOT_AUTH_ACTION')));
                }
                //检查当前操作是否需要认证
            ...
            if (!session(config('app.USER_AUTH_KEY'))) {
                if (config('app.GUEST_AUTH_ON')) {
                    ...
                    if (!session('_ACCESS_LIST'))
                        // 保存游客权限
                        RBAC::saveAccessList(config('app.GUEST_AUTH_ID'));
                } else {
                    // 禁止游客访问跳转到认证网关
                    redirect(url(config('app.USER_AUTH_GATEWAY')));
                }
            }
        }
        return true;
    ...
    static public function AccessDecision($appName = 'admin')
    {
        ...
        if (self::checkAccess()) {
            ...
            if (empty(session(config('app.ADMIN_AUTH_KEY')))) {
                if (config('app.USER_AUTH_TYPE') == 2) {
                    //加强验证和即时验证模式 更加安全 后台权限修改可以即时生效
                    //通过数据库进行访问检查
                    $accessList = self::getAccessList(session(config('app.USER_AUTH_KEY')));
                } else {
                    // 如果是管理员或者当前操作已经认证过，无需再次认证
                    if (session($accessGuid)) {
                        return session($accessGuid);
                    }
                ...
                //栏目权限
                if (!session(config('app.USER_CONTENT_KEY'))) {
                    $table = array('role' => config('app.RBAC_ROLE_TABLE'), 'user' => config('app.RBAC_USER_TABLE'));
                    $sql = "select role.typeids from " .
                        $table['role'] . " as role," .
                        $table['user'] . " as user" .
                        " where user.user_id=" . session(config('app.USER_AUTH_KEY')) . " and user.role_id=role.id and role.status=1";
                    //echo $sql;
                    $rs = Db::query($sql);
                    if ($rs) {
                        session(config('app.USER_CONTENT_KEY'), $rs[0]['typeids']);
                    }
                }
                //判断是否为组件化模式，如果是，验证其全模块名
                $module = defined('P_MODULE_NAME') ? P_MODULE_NAME : MODULE_NAME;
                //追影改写默认为无需认证 只有设置了才验证因为没有添加进node的实在太多
                $model_node = Db::table(config('app.RBAC_NODE_TABLE'));
                $t2 = $model_node->whereRaw("upper(`name`)='" . strtoupper($module) . "' and level=2")->find();
                if (!$t2) {
                    session($accessGuid, false);
                    return false;
                }
                $t = $model_node->whereRaw("upper(`name`)='" . strtoupper(ACTION_NAME) . "' and level=3 and pid=" . $t2['id'])->find();
    ...
    {
        // Db方式权限数据
        $table = array('role' => config('app.RBAC_ROLE_TABLE'), 'user' => config('app.RBAC_USER_TABLE'), 'access' => config('app.RBAC_ACCESS_TABLE'), 'node' => config('app.RBAC_NODE_TABLE'));
    ...
    {
        // Db方式
        $table = array('role' => config('app.RBAC_ROLE_TABLE'), 'user' => config('app.RBAC_USER_TABLE'), 'access' => config('app.RBAC_ACCESS_TABLE'));
