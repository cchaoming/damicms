...
class Image {//类定义开始
     ...
     * @throws ThinkExecption
      +----------------------------------------------------------
     */
     static public function water($source, $water, $savename=null, $alpha=0) {
        //检查文件是否存在
        ...
        imagejpeg($sImage, $savename);//第3个参数即使质量大小，因为只有imagejpeg支持这个参数
        } else {
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
        $randval = MiString::rand_string($length, $mode);
        $fontPath= dirname(__FILE__) . "/verify/ttfs/1.ttf";
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            imagestring($im, 5, $i * 15 + 5, mt_rand(1, 8), $randval{$i}, $stringColor);

           // imagettftext($im,14,10,$i*15+5,mt_rand(16,26),$stringColor,$fontPath,$randval{$i});
        }
//        @imagestring($im, 5, 5, 3, $randval, $stringColor);
    ...
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='simhei.ttf', $verifyName='verify') {
        ...
        $code = MiString::rand_string($length, 4);
