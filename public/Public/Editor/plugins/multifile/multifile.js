...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		self.div = K(options.container).html([
			...
			'<div class="ke-swfupload-body" id="ke-multiupload"></div>',
			'</div>'
		].join(''));
        ...
        var file_types = options.fileTypes.split(';');
		...
		var settings = {
			...
			root_path        :   options.zyUploadPath,
			url              :   options.uploadUrl,  // 上传文件的路径
			fileType         :   file_types,
		...
		$("#ke-multiupload").zyUpload(settings);
	},
	getUrlList : function() {
		var self = this;
		return self.files;
...
KindEditor.plugin('multifile', function(K) {
	var self = this, name = 'multifile',
		...
		zyUploadPath = self.pluginsPath + '/multifile/zyupload/',
		imageSizeLimit = K.undef(self.imageSizeLimit, '10MB'),
		imageUploadLimit = K.undef(self.imageUploadLimit, 10),
		filePostName = K.undef(self.filePostName, 'imgFile'),
		lang = self.lang(name + '.');
	dynamicLoadCss(self.basePath + '/plugins/multifile/zyupload/skins/zyupload-1.0.0.min.css');
	self.plugin.multiFileDialog = function(options) {
		...
		var dialog = self.createDialog({
			...
			previewBtn : {
				name : lang.insertAll,
				click : function(e) {
					console.log('geturlllist');
					console.log(swfupload.getUrlList());
